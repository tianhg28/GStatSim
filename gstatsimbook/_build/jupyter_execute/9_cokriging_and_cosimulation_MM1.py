...
# Based on our post-simulation analysis, it appears that the MM1 simulation exaggerates the roughness and correlation betweehn the bed and the surface. These are known issues with MM1 and is something to watch out for. This over-correlation can sometimes create simulation artifacts as well. Generally speaking, MM1 works best if the secondary data has a higher variance (i.e. is rougher) than the primary data.
